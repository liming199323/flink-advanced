...
=================
   ...
   * [转换操作（Transformations）](#转换操作transformations)
      ...
      * [Filter](#filter)
      * [Projection of Tuple DataSet](#projection-of-tuple-dataset)
      * [Transformations on Grouped DataSet](#transformations-on-grouped-dataset)
      * [Reduce on Grouped DataSet（减少分组数据集）](#reduce-on-grouped-dataset减少分组数据集)
      * [GroupReduce on Grouped DataSet](#groupreduce-on-grouped-dataset)
      * [可组合的GroupReduce函数](#可组合的groupreduce函数)
      * [GroupCombine on a Grouped DataSet](#groupcombine-on-a-grouped-dataset)
      * [Aggregate on Grouped Tuple DataSet（聚合在分组元组数据集）](#aggregate-on-grouped-tuple-dataset聚合在分组元组数据集)
      * [MinBy / MaxBy on Grouped Tuple DataSet](#minby--maxby-on-grouped-tuple-dataset)
      * [Reduce on full DataSet（减少完整的DataSet）](#reduce-on-full-dataset减少完整的dataset)
      * [GroupReduce on full DataSet](#groupreduce-on-full-dataset)
      * [GroupCombine on a full DataSet](#groupcombine-on-a-full-dataset)
      * [Aggregate on full Tuple DataSet](#aggregate-on-full-tuple-dataset)
      * [MinBy / MaxBy on full Tuple DataSet](#minby--maxby-on-full-tuple-dataset)
...
该笔记专栏源码与 [github/flink-advanced](https://github.com/GourdErwa/flink-advanced) 同步   
...
```
## Aggregate on Grouped Tuple DataSet（聚合在分组元组数据集）
